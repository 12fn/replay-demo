// Public synthetic schema tests. See tests/fixtures/README.md for provenance and limits.
...
import { exportFiles } from "../../scripts/export-catalog-ontology.ts";

import {syntheticTrial, syntheticFingerprint, SYNTHETIC_TRIAL as TRIAL, SYNTHETIC_GAME as GAME, SYNTHETIC_ROUNDS as ROUNDS} from "../fixtures/synthetic-trial";
...
describe("recorded-trial schema projection using synthetic input", () => {
  it("validates a synthetic game-shaped fixture and links every decision to its exact prompt, snapshot, reply and provider record", () => {
    expect(checkGraphIntegrity(graph)).toEqual([]);
    const t = node(trialNodeId(GAME));
    expect(t.properties).toMatchObject({ seed: "SYNTH001", scenarioId: "taiwan-strait/1", map: "synthetic-map", blueModel: "gpt-5.6-sol", redModel: "claude-opus-5", actualRecordedGame: true, authoredExample: false });
    ...
    expect(decisions).toHaveLength(ROUNDS * 2);
    for (const d of decisions) {
      ...
      expect(d.properties.executedTick).toBe(d.properties.choice === 'hold' ? null : d.properties.observedTick);
    ...
    expect(final.properties).toMatchObject({ winner: null, reason: null, ticksSimulated: 45 + ROUNDS * 270, finalFingerprint: syntheticFingerprint(ROUNDS) });
    expect(final.properties.engineReconstructionReceipt).toBeNull();
...
describe("raw game validation rejects bad input", () => {
  ...
  it("enforces bounds and checks the engine receipt against the final state", () => {
    ...
    const summary = JSON.parse(input.files.find(f => f.path === 'final/summary.json')!.text);
    // Authored receipt-shaped input tests binding only; no reconstruction is asserted.
    const validReceipt = {path: `evidence/platform/${TRIAL}-main-review.json`, text: JSON.stringify({
      status: 'passed', trial: TRIAL, finalFingerprint: summary.reconstruction.finalFingerprint,
      ticks: summary.ticksSimulated, actualProviderCalls: ROUNDS * 2, sourceHashesVerified: summary.sources.length,
      fixtureNotice: 'Synthetic receipt-shaped test input; no engine or provider run.'})};
    expect(validateTrialInput({...clone(input), engineReceipt: validReceipt}).engineReceipt).not.toBeNull();
    receipt.engineReceipt = {...validReceipt, text: validReceipt.text.replace(summary.reconstruction.finalFingerprint, '1'.repeat(64))};
...
describe("behaviour observations", () => {
  ...
  it("are mechanical counts with resolvable numerators and denominators", () => {
    expect(obs("blue", "wait-rate")).toMatchObject({ numerator: 1, denominator: ROUNDS, proportion: 0.125 });
    expect(graph.observations.every(o => o.interval95 === null)).toBe(true);
    expect(obs("red", "order-submitted-rate")).toMatchObject({ numerator: 7, denominator: ROUNDS });
    expect(obs("blue", "troop-share-0.2")).toMatchObject({ numerator: 5, denominator: 6 });
    expect(obs("red", "intent-construction-share")).toMatchObject({ numerator: 2, denominator: 7 });
    expect(obs("red", "construction-completed-rate")).toMatchObject({ numerator: 1, denominator: 2 });
    expect(obs("blue", "stations-controlled-at-end")).toMatchObject({ numerator: 1, denominator: 2, interval95: null });
    expect(obs("blue", "transport-landed-rate")).toMatchObject({ numerator: 1, denominator: 2 });
    expect(obs("blue", "intent-transport-share")).toMatchObject({ numerator: 2, denominator: 7 });
...
describe("exports", () => {
  it("builds bounded Graphiti-shaped batches that are clearly not native ingestion", () => {
    ...
    expect(messages).toHaveLength(3 + 1 + ROUNDS);
  ...
  it("writes deterministic portable files that re-verify after a JSON round trip", () => {
    ...
    expect(exportFiles(buildOntologyGraph({ catalog: createPresetCatalog(), trials: [syntheticTrial()] }))).toEqual(files);
...
it('imports a synthetic side-swapped game-shaped fixture with each provider format following its model',()=>{
 const swapped=syntheticTrial(true);

/**
 * Add two preset reference reads to native policy and one existing private member helper.
 * Run serially with other Commander logins:
 *   run_logged.py LABEL -- tsx scripts/platform/enable-tomo-catalog.ts VERSION inspect|apply commander|intelligence
 * Offline checks: tsx scripts/platform/enable-tomo-catalog.ts --self-test
...
const NEW_TOOLS=['search_catalog','get_catalog_record'].map(n=>`kz_${SERVER}_${n}`);
...
const READS = ['list_exercises', 'get_exercise_state', 'get_station_objectives', 'get_team_assessments',
  'get_key_moments', 'get_replay_provenance', 'search_practice_history', 'search_practice_details','search_catalog','get_catalog_record'].sort();
...
export function withCatalogTools(content: Json): Json {
  check(content && ['REPLAY staff watch helper','REPLAY intelligence staff helper'].includes(content.name) && content.capability_ceiling === 'write', 'observer-definition');
  ...
  unchanged(content.posture.platform_tool_allowlist.filter((n: string) => !NEW_TOOLS.includes(n)).sort(), ALL_REPLAY.filter(n => !NEW_TOOLS.includes(n)), 'existing-helper-tool-set');
  for(const name of NEW_TOOLS)check(content.posture.platform_tool_allowlist.filter((n:string)=>n===name).length<=1,'duplicate-catalog-tool');
  const next = structuredClone(content);
  for(const name of NEW_TOOLS)if(!next.posture.platform_tool_allowlist.includes(name))next.posture.platform_tool_allowlist.push(name);
  return next;
}
...
export function assertReplaySurface(replay: Json[]) {
  unchanged(replay.map(t => t.id).sort(), ALL_REPLAY, 'eleven-replay-tools-required');
  check(replay.filter(t => t.id !== WATCH).every(t => t.capability === 'read' && !t.requires_confirm), 'ten-reads-required');
...
export function assertCatalogScope(tools: Json[]) {
...
async function main(version: string, mode: string, role: string) {
  check(/^\d+\.\d+\.\d+$/.test(version) && ['inspect', 'apply'].includes(mode), 'usage-version-inspect-or-apply');
  check(['commander','intelligence'].includes(role),'member-role');
  const expectedName=role==='commander'?'REPLAY staff watch helper':'REPLAY intelligence staff helper';
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  const artifact = `evidence/platform/tomo-catalog-${version}-${role}-${mode}-${stamp}.json`;
  ...
  const agentId:string=role==='commander'?reference.agent.id:JSON.parse(fs.readFileSync('evidence/platform/tomo-intelligence-helper-apply-2026-09-15T04-42-54-180Z.json','utf8')).helper.id;
  ...
  const save = (status: string) => {
    const proof = { at: new Date().toISOString(), version, mode, status, phase, agentId, role, catalogToolIds: NEW_TOOLS,
  ...
  async function login(who: 'commander' | 'admin' | 'owner', enter = true) {
    ...
    try {
      ...
      receipts.push({ operation: 'normal-login', identity: who==='commander'?role:who, subject: me.identity.userId, enteredWorkroom: entered });
      return {
        ...
        async tomo(method: 'GET' | 'PUT', target: string, body?: unknown, expectedStatus = 200): Promise<Json> {
          ...
          const paths = ['/api/auth/me', '/api/agents/capability-catalog', '/api/ops/kamiwaza-tools',
            `/api/agents/${agentId}`, `/api/agents/${agentId}/definition`, `/api/agents/capability-catalog?agent_id=${agentId}`, ...NEW_TOOLS.map(n=>`/api/ops/kamiwaza-tools/${n}`)];
          check(paths.includes(target) && (method === 'GET' || mode === 'apply'
            && [ `/api/agents/${agentId}`, ...NEW_TOOLS.map(n=>`/api/ops/kamiwaza-tools/${n}`) ].includes(target)), 'tomo-mutation-scope');
          ...
          receipts.push({ operation: 'tomo-request', identity: who==='commander'?role:who, method, path: target, status: response.status,
  ...
  async function memberInspection() {
    ...
    check(card.id === agentId && card.name === expectedName && card.visibility === 'private'
    ...
    withCatalogTools(definition.content);
  ...
  try {
    ...
    if (mode === 'apply') {
      ...
      try {
        ...
        check(replay.filter((t:Json)=>!NEW_TOOLS.includes(t.id)).every((t:Json)=>t.enabled),'existing-tools-not-enabled');
        let current=selection;let changed=0;
        for(const name of NEW_TOOLS){
          const item=current.items.find((t:Json)=>t.id===name);check(item,'catalog-tool-id');
          if(!item.enabled){await active.tomo('PUT',`/api/ops/kamiwaza-tools/${name}`,{enabled:true,expected_version:current.version});changed++;}
          const after=await active.tomo('GET','/api/ops/kamiwaza-tools');
          const updated=after.items.find((t:Json)=>t.id===name);check(updated?.enabled===true,'catalog-policy-not-enabled');assertOnlyEnabledChanged(item,updated);
          unchanged(current.items.filter((t:Json)=>t.id!==name),after.items.filter((t:Json)=>t.id!==name),'unrelated-tool-settings-changed');
          unchanged(current.root,after.root,'tool-root-settings-changed');
          check(after.version===current.version+(item.enabled?0:1),'tool-selection-version');current=after;
        }
        checks.adminSelection={beforeVersion:selection.version,afterVersion:current.version,changedTools:changed,enabledReplayTools:current.items.filter((t:Json)=>t.server_id===SERVER&&t.enabled).map((t:Json)=>t.id),rootPreserved:true,unrelatedSettingsPreserved:true};
        save('running');
      ...
      const content = withCatalogTools(current.definition.content), changed = !equalContent(current.definition.content, content);
      if (changed) {
      ...
      assertCatalogScope(scoped.tools);
...
if(process.argv[1]&&import.meta.url===pathToFileURL(path.resolve(process.argv[1])).href){
 if(process.argv[2]==='--self-test'){
  const base={name:'REPLAY staff watch helper',capability_ceiling:'write',posture:{platform_tool_allowlist:ALL_REPLAY.filter(n=>!NEW_TOOLS.includes(n))},instructions:'preserve'};
  const next=withCatalogTools(base);assert.equal(next.posture.platform_tool_allowlist.length,11);assert.equal(base.posture.platform_tool_allowlist.length,9);assert.equal(next.instructions,'preserve');assert.deepEqual(withCatalogTools(next),next);
  assert.throws(()=>withCatalogTools({...base,posture:{platform_tool_allowlist:[...base.posture.platform_tool_allowlist,'unexpected']}}));
  assert.throws(()=>assertOnlyEnabledChanged({id:'a',enabled:false,requires_confirm:false},{id:'a',enabled:true,requires_confirm:true}));
  console.log(JSON.stringify({selfTest:'passed',modelCalls:0}));
 }else await main(process.argv[2]??'',process.argv[3]??'',process.argv[4]??'');
}

...
try:
 ...
 print(json.dumps({'status':r.status,'signedIdentity':safe,'user':{k:body.get(k) for k in ['sub','username','roles']},'bridge':{'status':urllib.request.urlopen(urllib.request.Request('http://replay-server.kamiwaza-extensions.svc:5181/internal/graph/v1/models',headers={'Authorization':'Bearer '+token})).status},'embedding':{'status':embedding.status,'dimensions':[len(v) for v in vectors],'relevantSimilarity':dot(vectors[0],vectors[1]),'unrelatedSimilarity':dot(vectors[0],vectors[2]),'paidInference':False}}))
...
result=json.loads(r.stdout);result['at']=datetime.datetime.now(datetime.timezone.utc).isoformat();pathlib.Path('evidence/platform/graphiti-bridge-qualification.json').write_text(json.dumps(result,indent=2));print(json.dumps(result))

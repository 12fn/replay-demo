...
const STEPS = [
  { title: 'Decision', detail: 'Decision record', minutes: '1 min' },
  { title: 'Evidence', detail: 'Sources and contradictions', minutes: '2 min' },
  { title: 'AI analysis', detail: 'Saved analysis and citations', minutes: '2 min' },
  { title: 'Review note', detail: 'Record an instructor review', minutes: '1 min' },
  { title: 'Practice & export', detail: 'Practice branch and evidence export', minutes: '4 min' },
];
...
export function ShowcaseView({ ctx }: { ctx: ViewContext }) {
  ...
  return <div className="showcase-page">
    <PageHeading title="Instructor case"
    ...
    <section className="case-hero" aria-label="Prepared case briefing">
      <div className="case-intro">
        ...
        <p>A participant committed resources based on duplicate and contradictory reports. Review the source timeline, add a review note, or test another order in a practice branch.</p>
      ...
      <div className="case-route" aria-label="Learning sequence">
        <div><span className="route-symbol"><FileText size={19} aria-hidden="true" /></span><span><strong>Decision record</strong><small>Orders and reports available at the time</small></span></div>
        <div><span className="route-symbol"><Waypoints size={19} aria-hidden="true" /></span><span><strong>Source relationships</strong><small>Duplicate claims and conflicting observations</small></span></div>
        <div><span className="route-symbol"><GitBranch size={19} aria-hidden="true" /></span><span><strong>Practice branch</strong><small>Alternative orders in a separate exercise</small></span></div>
      </div>
    </section>
    <InlineError message={error} />
    {message && <p className="notice-success" role="status">{message}</p>}
    {!data && !error && <p className="empty" role="status">Loading the prepared case…</p>}
    {data && !selected && <div className="setup-notice"><div><h3>Set up the Crosscurrent case</h3><p>Install the bundled fictional case once. Your subsequent notes and practice branches will be retained.</p></div><button className="btn btn-primary" disabled={busy} onClick={() => void run(async () => { await request('/api/showcase/prepare', 'POST'); await ctx.refresh(); await load(); setMessage('Prepared case is ready.'); })}>Prepare rehearsal (one-time setup)<ArrowRight size={16} aria-hidden="true" /></button></div>}
    <div className="case-workspace">
      ...
      <section className="case-stage" aria-label={STEPS[step].title}>
        ...
        {step === 0 && <div className="stage-columns"><div className="stage-copy"><h3>Orders and available reports</h3><p>Open the original order alongside the reports available to the participant. Later corrections stay separate from what they could have known then.</p><button className="btn btn-primary" disabled={!selected || busy} onClick={() => void run(review)}>Open decision and source timeline<ArrowRight size={16} aria-hidden="true" /></button></div><aside className="teaching-prompt"><span className="eyebrow">Ask the room</span><blockquote>“What evidence would you want before committing more resources?”</blockquote></aside></div>}
    ...
    <footer className="case-platform-note"><ShieldCheck size={18} aria-hidden="true" /><p><strong>Native workroom services.</strong> Kamiwaza supplies native identity, managed archives, and recorded model/tool integrations. </p><button className="link" onClick={() => ctx.navigate('platform')}>Open platform details<ArrowRight size={14} aria-hidden="true" /></button></footer>
  </div>;
}

...
// REPLAY patch (2026-09-13, see docs/process/naval-isolation-notes.md):
// upstream keeps the rebuild stagger in a process-global static, so every
// game hosted in one process (live exercises, replays, branches) shares and
// advances the same counter. REPLAY runs many games per process and needs
// each game's simulation to depend only on its own recorded inputs, so the
// counter is kept per Game instance instead. Semantics inside one game are
// unchanged: successive transports still receive 0, 1, 2, ... mod
// STAGGER_SPREAD. Original source: OpenFrontIO (AGPL-3.0), pinned commit in
// vendor/openfront/UPSTREAM.json.
const staggerCounters = new WeakMap<Game, number>();
function nextTransportStagger(game: Game): number {
  const count = staggerCounters.get(game) ?? 0;
  staggerCounters.set(game, count + 1);
  return count % WaterPathFinder.STAGGER_SPREAD;
}
/** Transports initialised so far in `game`; diagnostics only. */
export function transportStaggerCount(game: Game): number {
  return staggerCounters.get(game) ?? 0;
}
...
export class TransportShipExecution implements Execution {
  ...
  // REPLAY patch: `private static _staggerCounter = 0;` replaced by the
  // per-game counter above.
  ...
  init(mg: Game, ticks: number) {
    ...
    // REPLAY patch: game-local stagger (was the process-global static).
    const stagger = nextTransportStagger(mg);

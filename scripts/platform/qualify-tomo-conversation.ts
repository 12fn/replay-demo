/** One native member conversation. Usage: VERSION [ATTEMPT_SUFFIX]. No submission retries. */
...
const artifact = `evidence/platform/tomo-conversation-${attempt}.json`;
const privateEvents = `data/platform/tomo-conversation-${attempt}-events.jsonl`;
const privateDetail = `data/platform/tomo-conversation-${attempt}-detail.json`;
for (const path of [artifact, privateEvents, privateDetail])
...
const proof: Record<string, any> = {
  ...
  exerciseId: '65a6e4ee-c449-4472-8b5a-f81dc251402a', tick: 500, status: 'starting',
...
const errorText = (error: unknown) => error instanceof Error ? error.message : 'Unknown qualification error';
...
try {
  ...
  const user = users.find((u: any) => u.role === 'commander');
  assert(user);
  ...
  const message = `Use kz_replay-tools_replay_get_exercise_state to inspect fictional exercise ${proof.exerciseId} at recorded tick ${proof.tick}. In under 100 words, state both sides' recorded forces and holdings, and cite the returned tick and fingerprint. Use the real tool result; do not infer a skill rating or take any action.`;
  ...
  const accepted = await withDeadline(async signal => {
    const response = await client.requestRaw(`${prefix}/${roomId}/inputs`, {
      ...
      platform_tool_names: ['kz_replay-tools_replay_get_exercise_state'],
